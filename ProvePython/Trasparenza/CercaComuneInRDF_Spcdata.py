...
with open(nomeFileCSV, 'rb') as csvfile:
    ...
    for row in reader:
        ...
        if nriga == 1 or intestazione == 'n':
            ...
            for comune in g.subjects(predicate=rdflib.RDF.type, object=rdflib.URIRef("http://spcdata.digitpa.gov.it/Comune")):
                nomeComune = g.value(comune, rdflib.RDFS.label).toPython().upper().replace('-', ' ')
                n = n + 1
                # print "nomeComune = ",nomeComune, " (", codiceComune,")"
                if nomeComune == val_input:
                    codiceComune = comune[-4:]
                    print comune, "è un comune"
                    print "nomeComune = ", nomeComune
                    print "TROVATO --", codiceComune, "--"
